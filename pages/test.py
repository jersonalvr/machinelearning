...
def show_test():
    ...
    st.write(f"Modelo cargado: **{uploaded_file.name.split('.')[0]}**")
    
    # Obtener y mostrar el intercepto si está disponible
    if hasattr(model, 'intercept_'):
        intercept = model.intercept_
        # Manejar interceptos múltiples (por ejemplo, en clasificación multiclase)
        if isinstance(intercept, np.ndarray):
            intercept = intercept.tolist()
        st.write(f"**Intercepto:** {intercept}")
    else:
        st.write("El modelo no tiene un intercepto.")
